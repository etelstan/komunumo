...
The `onApplicationStarted()` method is invoked after *Komunumo* has initialized but before it starts accepting requests. Currently, plugins must be part of the *Komunumo* source tree. External JAR-based plugins are not yet supported but may be added in a future version.
